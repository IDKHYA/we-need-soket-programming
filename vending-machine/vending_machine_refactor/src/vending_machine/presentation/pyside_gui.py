...
from vending_machine.domain.models import VALID_DENOMINATIONS
...
DENOMS = list(VALID_DENOMINATIONS)
THEME = {
    "navy": "#0A2B68",
    "navy_dark": "#071C48",
    "blue": "#318CFF",
...
class BackendController:
    ...
    def _commit(self, state, session, events):
        commit_local_and_publish(self.repo, self.workbook_path, state, session, events)
...
class VendingMachineWindow(QMainWindow):
    ...
    def _set_machine_paused(self, paused: bool):
        ...
        if paused:
            self.display_panel.set_status("관리자 모드 실행 중입니다. 자판기 입력을 잠시 멈춥니다.", ok=False)
            self.display_panel.set_pickup("관리자 모드가 끝나면 다시 이용할 수 있습니다.")
        self._render_products()
...
def _find_workbook() -> Path:
    ...
    for path in candidates:
        if path.exists():
            return path
    # 없으면 bootstrap 자동 실행
    _auto_bootstrap(data_dir)
    return data_dir / "vending_machine_template.xlsx"


def _auto_bootstrap(data_dir: Path) -> None:
    """데이터 파일이 없을 때 bootstrap_workbook.py를 자동으로 실행합니다."""
    import importlib.util
    import subprocess
    bootstrap = _PROJECT_ROOT / "scripts" / "bootstrap_workbook.py"
    if bootstrap.exists():
...
def main(argv: list[str] | None = None) -> int:
    ...
    if argv:
        ...
        if not workbook.is_absolute():
            workbook = _PROJECT_ROOT / "data" / workbook
    else:
